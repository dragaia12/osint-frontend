...
export async function toggleDossier(dossier: Dossier): Promise<Dossier> {
  ...
  const { data, error } = await supabase.from("dossiers").update({ statut }).eq("id", dossier.id).select().single();
  if (error) throw error;
  return data as Dossier;
}
...
export async function getDashboardData() {
  const [dossiers, searches, entities, recent] = await Promise.all([
    ...
    supabase.from("recherches").select("id,query,input_type,nb_resultats,duree_ms,created_at").order("created_at", { ascending: false }).limit(10),
  ]);
  return {
